...
export const Header = () => {
  ...
  return (
    <>
      <header className="m-0 py-4 w-full rounded-none border-none fixed top-0 left-0 z-50 shadow-none" style={{
        ...
        <div className="flex items-center justify-between text-white max-w-[1400px] mx-auto px-4">
          <Link to="/" className="flex">
            <span className="text-white font-bold xl:text-3xl text-2xl">
              Photo <span className="text-white">Idol.</span>
            </span>
          </Link>

          <div className="text-sm flex gap-5 items-center">
            <div className={`flex items-center ${url === "/search" ? "hidden" : ""}`}>
              <button type="button" className="lg:hidden">
                <BsSearch size={17} />
              </button>
              <form className="fixed top-12 right-0 left-0 px-4 mt-3 lg:mt-0 lg:relative md:left-auto lg:top-0">
                <div
                  className="flex items-center rounded h-[48px] ps-5 w-full outline-none text-dark bg-white md:min-w-[500px] max-w-full"
                  style={{
